app.factory('DrawService', ['TilingData', '$window', '$timeout', '$translate', 'ToastService',
    ...
    function init(isExport, mosaicIdx) {
        ...
        var tilesHeight = 0;
        self.mosaicsToRender.forEach( function(mosaic) {
            maxWidth = Math.max(mosaic.tiles[0].width, maxWidth);
            maxHeight = Math.max(mosaic.tiles[0].height, maxHeight);
        ...
        var ratioH = self.divWidth / maxWidth;
        ratioH *= 0.85;

        // Find a ratio to fit vertically
        var ratioV = self.divWidth / maxHeight;
        ratioV *= 0.80;
