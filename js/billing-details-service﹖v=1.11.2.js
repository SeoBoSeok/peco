    ...
    function($http, ClientInfo, AuthService, CutListCfg, $q) {
        ...
        self.postInvoiceCustomerDetail = function () {
            ...
            if (self.shouldSendInvoice) {
                // Add country code to VAT number if required
                if (!!self.invoiceCustomerDetail.taxRegistrationNumber && self.invoiceCustomerDetail.taxRegistrationNumber.length >= 2) {
                    var vat = self.invoiceCustomerDetail.taxRegistrationNumber || "";
                    if (!isNaN(vat.charAt(0)) && !isNaN(vat.charAt(1))) {
                        vat = self.invoiceCustomerDetail.countryCode + vat;
                    }
                    self.invoiceCustomerDetail.taxRegistrationNumber = vat;
                }
        ...
        self.validateVat = function () {
            var vatToValidate = self.invoiceCustomerDetail.taxRegistrationNumber || "";
            if (!isNaN(vatToValidate.charAt(0)) && !isNaN(vatToValidate.charAt(1))) {
                vatToValidate = self.invoiceCustomerDetail.countryCode + vatToValidate;
            }
