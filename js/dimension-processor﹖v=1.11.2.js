app.factory('DimensionProcessor', function() {
    ...
    var UnitsEnum = Object.freeze({"generic": 1, "mm": 2, "cm": 3, "inches": 4, "feet_inches": 5, "feet_inches_frac": 6, "inches_frac": 7});
...
function toFraction(decimal, resolution) {
    ...
    var length = decimal.toString().length - 2;

    var denominator = Math.pow(10, length);
    ...
    if (numerator.toString().length > 3 || denominator.toString().length > 3) {
        var digitsToRemove = Math.min(numerator.toString().length, denominator.toString().length);
        digitsToRemove -= 2;
